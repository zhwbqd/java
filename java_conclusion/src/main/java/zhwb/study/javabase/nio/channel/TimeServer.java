 ...
 * code implements an RFC 868 listener to provide time service. The defined port
 * for time service is 37. On most unix systems, root privilege is required to
 * bind to ports below 1024. You can either run this code as root or provide
 * another port number on the command line. Use "-p port#" with TimeClient if
 * you choose an alternate port. Note: The familiar rdate command on unix will
 * probably not work with this server. Most versions of rdate use TCP rather
 * than UDP to request the time.
 */
public class TimeServer {
	...
	public void listen() throws Exception { // Allocate a buffer to hold a long
											// value
